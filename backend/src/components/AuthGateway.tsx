...
export default function AuthGateway({ onAuthSuccess }: Props) {
  ...
  // OAuth authentication handler and window postMessage listeners
  React.useEffect(() => {
    const handleOAuthMessage = async (event: MessageEvent) => {
      const origin = event.origin;
      // Accept local development, staging and live run.app endpoints
      if (!origin.endsWith('.run.app') && !origin.includes('localhost') && !origin.includes('127.0.0.1')) {
        return;
      }

      if (event.data?.type === 'OAUTH_AUTH_SUCCESS') {
        const oAuthUserId = event.data?.userId;
        if (oAuthUserId) {
          setLoading(true);
          setErrorMessage(null);
          try {
            const res = await apiFetch(`/api/users/${oAuthUserId}`);
            if (res.ok) {
              const uData = await res.json();
              onAuthSuccess(uData);
            } else {
              setErrorMessage('Failed to resolve authenticated social profile details.');
            }
          } catch (err) {
            setErrorMessage('Network timeout resolving social profile.');
          } finally {
            setLoading(false);
          }
        }
  ...
  const handleOAuthLogin = async (provider: 'google' | 'facebook') => {
    ...
    setErrorMessage(null);
    try {
      const res = await apiFetch(`/api/auth/${provider}/url`);
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || `Failed to fetch callback url for ${provider}`);
      }

      // Open OAuth vendor consent prompt directly in self-closing viewport popup
      const width = 500;
      const height = 650;
      ...
      const authWindow = window.open(
        data.url,
        `soko_oauth_${provider}`,
        `width=${width},height=${height},left=${left},top=${top},scrollbars=yes,status=no`
      );

      if (!authWindow) {
        setErrorMessage('Sokos Action Blocked: Popups are disabled on your browser. Please allow popups to utilize social authentication.');
      }
    } catch (err: any) {
      setErrorMessage(err.message || 'Verification link failed.');
    } finally {
      // Keep loading spinner contextual or clear it so user can click other things if popup is block/failed
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-zinc-900/90 border border-zinc-800/80 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden backdrop-blur-md">
        ...
        {activeMode === 'demo' && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="flex items-center my-4 font-mono">
              <div className="flex-grow h-px bg-zinc-800/80" />
              <span className="mx-3.5 text-[9px] uppercase tracking-widest text-zinc-500 font-bold">Or Connect Instantly</span>
              <div className="flex-grow h-px bg-zinc-800/80" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 font-sans">
              <button
                ...
                disabled={loading}
                className="flex items-center justify-center gap-2 py-3 px-4 rounded-2xl bg-zinc-950 hover:bg-zinc-850 border border-zinc-855 text-zinc-200 hover:text-white font-bold text-xs select-none transition-all cursor-pointer shadow-sm active:scale-[0.98]"
              >
          ...
          <form onSubmit={handleClassicLogin} className="space-y-4 animate-fade-in">
            ...
            <div className="space-y-3.5">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3.5 px-4 bg-emerald-500 hover:bg-emerald-400 disabled:bg-zinc-800 disabled:text-zinc-600 text-zinc-950 font-black rounded-2xl text-xs tracking-wide uppercase transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-[0_4px_12px_rgba(16,185,129,0.15)] animate-grow"
              >
              ...
              <div className="flex items-center my-4 font-mono">
                <div className="flex-grow h-px bg-zinc-800/80" />
                <span className="mx-3.5 text-[9px] uppercase tracking-widest text-zinc-500 font-bold">Or Instant Sign-In</span>
                <div className="flex-grow h-px bg-zinc-800/80" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 font-sans">
                <button
                  ...
                  onClick={() => handleOAuthLogin('google')}
                  disabled={loading}
                  className="flex items-center justify-center gap-2 py-3 px-4 rounded-2xl bg-zinc-950 hover:bg-zinc-850 border border-zinc-850 text-zinc-200 hover:text-white font-bold text-xs select-none transition-all cursor-pointer shadow-sm active:scale-[0.98]"
                >
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
        ...
        {activeMode === 'register' && (
          <form onSubmit={handleClassicRegister} className="space-y-4 animate-fade-in">
            ...
            <div className="space-y-3.5">
              ...
              <div className="flex items-center my-4 font-mono">
                <div className="flex-grow h-px bg-zinc-800/80" />
                <span className="mx-3.5 text-[9px] uppercase tracking-widest text-zinc-500 font-bold">Or Register With</span>
                <div className="flex-grow h-px bg-zinc-800/80" />
              </div>
