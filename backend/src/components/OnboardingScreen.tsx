...
import { NAIROBI_WARDS, Ward } from '../wards';
import { 
  MapPin, 
  UserCheck, 
  Shield, 
...
export default function OnboardingScreen({ currentUser, onOnboardSuccess }: Props) {
  ...
  const [chosenNeighborhood, setChosenNeighborhood] = useState<{
    ...
    name: NAIROBI_WARDS[0].name,
    latitude: NAIROBI_WARDS[0].lat,
    longitude: NAIROBI_WARDS[0].lon,
    desc: NAIROBI_WARDS[0].desc,
    subCounty: NAIROBI_WARDS[0].subCounty
  });
  const [customGpsName, setCustomGpsName] = useState('My GPS Location');
  const [detectingGps, setDetectingGps] = useState(false);
  const [gpsError, setGpsError] = useState<string | null>(null);
  const [wardSearchQuery, setWardSearchQuery] = useState('');
  const [selectedSubCountyFilter, setSelectedSubCountyFilter] = useState('All');

  const findClosestWard = (lat: number, lon: number) => {
    let closest = NAIROBI_WARDS[0];
    let minD = Infinity;
    for (const w of NAIROBI_WARDS) {
      const d = Math.sqrt(Math.pow(w.lat - lat, 2) + Math.pow(w.lon - lon, 2));
      if (d < minD) {
        minD = d;
        closest = w;
      }
  ...
  const handleDetectGps = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const name = `📍 GPS Near ${closest.name.replace(' Ward', '')}`;
        const newZone = {
          name,
          latitude,
          longitude,
          desc: `Real-time browser coordinates (Near ${closest.name}, ${closest.subCounty} Sub-County)`,
          subCounty: closest.subCounty
        };
          ...
          {step === 2 && (
            <div className="space-y-4 animate-fade-in font-sans">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-zinc-800/60 pb-3 mb-1">
                <label className="block text-[10px] font-black uppercase text-zinc-400 font-mono tracking-wider">Nairobi County Wards</label>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>{detectingGps ? 'Detecting GPS...' : '📍 PIN via Browser GPS'}</span>
                </button>
              </div>

              {gpsError && (
                <div className="p-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-300 text-[10px] font-mono leading-relaxed">
                  ⚠️ GPS Query Bypassed: {gpsError}. Please select your Nairobi Ward manually below.
                </div>
              )}

              {/* Hybrid Select and Search Controls */}
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[8px] font-black uppercase text-zinc-500 font-mono tracking-widest mb-1">FILTER SUB-COUNTY</label>
                  <select
                    value={selectedSubCountyFilter}
                    onChange={(e) => setSelectedSubCountyFilter(e.target.value)}
                    className="w-full bg-zinc-950 border border-zinc-800 text-[11px] text-zinc-200 px-2.5 py-1.8 rounded-xl outline-none focus:border-emerald-500 font-mono font-bold"
                  >
                ...
                <div>
                  <label className="block text-[8px] font-black uppercase text-zinc-500 font-mono tracking-widest mb-1">SEARCH WARD NAME</label>
                  <input
                    type="text"
                    value={wardSearchQuery}
                    onChange={(e) => setWardSearchQuery(e.target.value)}
                    placeholder="Search e.g. Kilimani..."
                    className="w-full bg-zinc-950 border border-zinc-800 text-[11px] text-zinc-100 px-2.5 py-1.5 rounded-xl outline-none focus:border-emerald-500 font-sans font-semibold"
                  />
                </div>
              </div>

              {/* Scrollable list of 85 wards */}
              <div className="grid grid-cols-1 gap-2 max-h-[160px] overflow-y-auto pr-1 border border-zinc-800/40 rounded-2xl p-1 bg-zinc-950/20">
                {NAIROBI_WARDS.filter((w) => {
                  const mSub = selectedSubCountyFilter === 'All' || w.subCounty === selectedSubCountyFilter;
                  const mSearch = w.name.toLowerCase().includes(wardSearchQuery.toLowerCase()) || 
                                  w.subCounty.toLowerCase().includes(wardSearchQuery.toLowerCase());
                  return mSub && mSearch;
                }).map((w) => {
                  const isChosen = chosenNeighborhood.name === w.name && chosenNeighborhood.latitude === w.lat;
                  return (
                    <button
                      key={w.name}
                      type="button"
                      ...
                      className={`w-full p-2.5 border rounded-xl text-left cursor-pointer transition flex items-center justify-between ${
                        isChosen 
                          ? 'border-emerald-500 bg-emerald-950/20 text-white' 
                          : 'border-zinc-850 bg-zinc-950/40 text-zinc-400 hover:bg-zinc-900 hover:text-white'
                      }`}
                    >
                      <div className="min-w-0 pr-2">
                        <div className="text-[11px] font-bold font-sans flex items-center gap-1.5 truncate">
                          <CircleDot className={`w-2.5 h-2.5 flex-shrink-0 ${isChosen ? 'text-emerald-400 fill-emerald-400' : 'text-zinc-600'}`} />
                          <span className="truncate">{w.name}</span>
                          <span className="text-[8px] bg-zinc-800 text-zinc-400 px-1.5 py-0.2 rounded-md font-mono uppercase tracking-wide">
                            {w.subCounty}
                          </span>
                        </div>
                        <div className="text-[9px] opacity-75 mt-0.5 truncate text-zinc-500">{w.desc}</div>
                      </div>
                      <div className="text-right font-mono text-[8px] text-zinc-500 flex-shrink-0">
                        <div>LAT {w.lat.toFixed(3)}</div>
                        <div>LON {w.lon.toFixed(3)}</div>
                      </div>
                    </button>
                  );
                })}
              ...
              {chosenNeighborhood.longitude !== undefined && (chosenNeighborhood.name.startsWith('📍') || !NAIROBI_WARDS.some(w => w.name === chosenNeighborhood.name)) && (
                <div className="space-y-1.5 p-3 rounded-2xl bg-zinc-950 border border-zinc-850 animate-fade-in text-left">
                  <div className="flex justify-between items-center">
                    <label className="block text-[9px] font-black uppercase text-zinc-400 font-mono tracking-widest">Customize GPS Pin Designation</label>
                    <span className="text-[8px] text-amber-500 bg-amber-500/10 px-1.5 py-0.5 rounded font-mono font-bold">HYBRID REAL-TIME CO-ORDINATES</span>
                  </div>
                  <input
                    type="text"
                    value={customGpsName}
                  ...
                  />
                  <p className="text-[9px] text-zinc-500 italic font-mono leading-tight">By customizing, Sokos establishes your direct coordinates for listings whilst anchoring your neighborhood to local county maps.</p>
                </div>
              )}

              <div className="flex items-center gap-2 text-[10px] text-zinc-400 bg-zinc-950/40 p-2.5 rounded-xl border border-zinc-850/80">
                <MapPin className="w-4 h-4 text-emerald-400 flex-shrink-0 animate-bounce" />
                <span className="truncate">Selected Localization: <strong className="text-emerald-400 font-bold">{chosenNeighborhood.name}</strong> • Range radius active.</span>
              </div>
            </div>
          )}
